...
setwd("C:/Users/gareyes3/Box/CPS Project- Farm to Facility/Leafy Green Paper/CSV Data/Review 2")
```
...
Binded_all$variable<-str_replace("Bef ", string = Binded_all$variable, replacement = "Before ")
order_progression2<-str_replace("Bef ", string = order_progression2, replacement = "Before ")

Binded_all$variable<-str_replace("Aft ", string = Binded_all$variable, replacement = "After ")
order_progression2<-str_replace("Aft ", string = order_progression2, replacement = "After ")
...
#For Presentations Same plot but binded
ggplot(data= Binded_all, aes(x= variable,y = mean, group= Type, color = Type, linetype=Type, fill = Type))+
  geom_line(size = 0.5)+
  geom_ribbon(aes(ymin = l95, ymax = h95), alpha = 0.2, size = 0.5)+
   scale_y_continuous(trans='log10',
                      labels = trans_format("log10", math_format(10^.x)))+
  labs(y= "Total Adulterant Cells in System", x="Process Stage")+
  #ggtitle("1) Random Contamination")+
...
ggsave("Progression-For-Presentations.jpeg", width = 4, height = 5, units = "in", dpi = 300)
...
FSdf$Name<-str_replace("Baseline NI", string = FSdf$System, replacement = "Baseline No-Intervention")
...
#####Plots for CPS PResentation 2023


FS_AI<-FSdf %>% 
  filter(System == "Baseline All-Intervention") %>% 
  filter(Name != "Baseline  AI")

unique(FS_AI$Name)

FS_AI$Name <-str_replace("Baseline  AI PHS4D", string = FS_AI$Name, replacement = "Preharvest 4 days")
FS_AI$Name <-str_replace("Baseline  AI PHS4H", string = FS_AI$Name, replacement = "Preharvest 4 Hours")
FS_AI$Name <-str_replace("Baseline  AI PHS Int", string = FS_AI$Name, replacement = "Intensive Preharvest")
FS_AI$Name <-str_replace("Baseline  AI HS", string = FS_AI$Name, replacement = "Harvest")
FS_AI$Name <-str_replace("Baseline  AI RS", string = FS_AI$Name, replacement = "Receiving")
FS_AI$Name <-str_replace("Baseline  AI FPS", string = FS_AI$Name, replacement = "Finished Product")
FS_AI$Name <-str_replace("Baseline  AI CS", string = FS_AI$Name, replacement = "Customer")


  
ggplot(data = FS_AI,aes(x = fct_reorder(Name, abs(FS)), y = FS))+
geom_col(fill = "#E69F00")+
coord_flip()+
ylab("Factor Sensitivity Index")+
xlab("")+
geom_hline(aes(yintercept = -3.416 ),linetype = "dashed",size =0.7)+
geom_hline(aes(yintercept = -3.416),size =0.7, color = "red", alpha = 0.5)

ggsave("Factor Sensitivity AI CPS23 Presentation.jpeg", width = 3, height = 2, units = "in", dpi = 300)


FS_NI<-FSdf %>% 
  filter(System == "Baseline No-Intervention") %>% 
  filter(Name != "Baseline NI")

unique(FS_NI$Name)

FS_NI$Name <-str_replace("Baseline NI PHS4D", string = FS_NI$Name, replacement = "Preharvest 4 days")
FS_NI$Name <-str_replace("Baseline NI PHS4H", string = FS_NI$Name, replacement = "Preharvest 4 Hours")
FS_NI$Name <-str_replace("Baseline NI PHS Int", string = FS_NI$Name, replacement = "Intensive Preharvest")
FS_NI$Name <-str_replace("Baseline NI HS", string = FS_NI$Name, replacement = "Harvest")
FS_NI$Name <-str_replace("Baseline NI RS", string = FS_NI$Name, replacement = "Receiving")
FS_NI$Name <-str_replace("Baseline NI FPS", string = FS_NI$Name, replacement = "Finished Product")
FS_NI$Name <-str_replace("Baseline NI CS", string = FS_NI$Name, replacement = "Customer")


  
ggplot(data = FS_NI,aes(x = fct_reorder(Name, abs(FS)), y = FS))+
...
ggsave("Factor Sensitivity NI CPS23 Presentation.jpeg", width = 3, height = 2, units = "in", dpi = 300)
...
Exposure_Chart$Sampling_Plan <-str_replace("PHS4D", string = Exposure_Chart$Sampling_Plan, replacement = "PHS 4D")
Exposure_Chart$Sampling_Plan <-str_replace("HTrad", string = Exposure_Chart$Sampling_Plan, replacement = "HS")
Exposure_Chart$Sampling_Plan <-str_replace("PHS4H", string = Exposure_Chart$Sampling_Plan, replacement = "PHS 4H")
Exposure_Chart$Sampling_Plan <-str_replace("PHSInt", string = Exposure_Chart$Sampling_Plan, replacement = "PHS Int")
Exposure_Chart$Sampling_Plan <-str_replace("RSTrad", string = Exposure_Chart$Sampling_Plan, replacement = "RS")
Exposure_Chart$Sampling_Plan <-str_replace("FPSTrad", string = Exposure_Chart$Sampling_Plan, replacement = "FPS")
...
unique(Exposure_Chart$Cont_Spread)
levels_1<-c("Baseline", "PHS 4D", "PHS 4H", "PHS Int", "HS", "RS", "FPS", "CS")
levels_2<-c("Baseline", "CS", "PHS4D", "HTrad", "RSTrad","PHS4H", "FPSTrad", "PHSInt")
...
## For Presentation
Exposure_Chart %>% 
  filter(Cont_Spread == "Random Uniform (Widespread 100% Cluster)") %>% 
  ggplot(aes(x = factor(Sampling_Plan, levels =levels_1 ) ,y = MeanComparison, group = Baseline_Scenario,fill = Baseline_Scenario))+
  theme_bw()+
  geom_line(aes(color =Baseline_Scenario), size  =0.7, alpha = 0.5)+
  geom_point(size = 2, position = position_dodge(width = 0),shape = 21, alpha = 1)+
  #theme(text = element_text(size=26), axis.text = element_text(size = 24))+
...
ggsave("Relative Efficacy-Presentation.jpeg", width = 4, height = 4, units = "in", dpi = 300)
```
