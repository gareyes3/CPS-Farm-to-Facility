...
##Powers
```{r}
Powers_df<-read_csv("Powers.csv")

Powers_df<- Powers_df %>%
  filter(Type != "None")

Powers_df$Type<-factor(Powers_df$Type, levels = c("PHS4D" ,"PHS4H", "PHSInt", "HS", "RS", "FPS", "CS"))
Powers_df$variable<-factor(Powers_df$variable, levels = c("No Intervention","Holding Only","PreCooling Only","Sanitation Only", "Prewash Only","Washing Only" , "All Intervention"))
Powers_df$ContS<-factor(Powers_df$ContS, levels = c("Random Uniform", "10% Cluster", "1% Cluster"))
...
ggsave("Powers3.tiff", width = 30, height = 16, units = "cm", dpi = 600)
...
hline_data <- data.frame(y = c(-3.416,0,-2.5970,-3.412
,-2.1759,-3.373,
-1.3206), System = factor(c("Baseline  AI", "Baseline NI", "No Holding", "No Precooling", "No PreWash", "No Sanitation", "No Washing")), 
                         stringsAsFactors = FALSE)

hline_data2 <- data.frame(y = c(0),Baseline_System = factor(c("Baseline")), 
                         stringsAsFactors = FALSE)

FSdf <- FSdf %>% mutate(ints3 = rep(c(-3.41,0,-0.805564771,0.015250639
...
#ALL Binded: this used to compared all other interventions
...
Cont_Samp_Point_Binded %>% 
  ...
  summarise(mean = mean(value), q2.5 = quantile(value, 0.05), q975 = quantile(value, 0.95)) %>% 
  filter(variable == "Before FPS")
...
```{r}
Exposure_Chart<-read_csv("Exposure_Chart_df-Final2.csv")
Exposure_Chart<-Exposure_Chart[-1]
...
ggplot(data = Exposure_Chart,aes(x = factor(Sampling_Plan, levels =levels_1 ) ,y = MeanComparison, group = Baseline_Scenario,fill = Baseline_Scenario))+
  theme_bw()+
  geom_line(aes(color =Baseline_Scenario ))+
  geom_point(size = 5, position = position_dodge(width = 0),shape = 21, alpha = 0.8)+
  theme(text = element_text(size=26), axis.text = element_text(size = 24))+
  theme(legend.text=element_text(size=16),legend.title=element_text(size=20))+
  #geom_col(data = Exposure_Chart, aes(x =factor(Sampling_Plan, levels =levels_2), y = log10cont))+
  facet_wrap(~Cont_Spread)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(y= "Relative Difference: TAC that leak through the system", x = "Sampling Plan", color= "Baseline Scenario", fill = "Baseline Scenario")+
  scale_y_continuous(labels = percent)+
  #geom_line(aes(group = Sampling_Plan),color="black",alpha = 0.7 ,arrow = arrow(length=unit(0.3,"cm"), ends="last", type #= "closed"))+
  #ggtitle("Relative TAC Exposure")+
  geom_hline(aes(yintercept = 0.2340680, linetype = "Maximum effect NI system"), size = 2, alpha = 0.5)+
  geom_hline(aes(yintercept = 0.6890373, linetype = "Maximum effect AI system"), size = 2,alpha = 0.5)+
  scale_linetype_manual(name ="", values = c('dotted','dashed')) 
