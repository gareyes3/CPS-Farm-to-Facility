...
hline_data <- data.frame(y = c(0, -3.4927), Baseline_System = factor(c("No Intervention", "All Intervention")), 
...
ggsave("contprog.png", width = 45, height = 25, units = "cm", dpi = 600)
...
#Exposure.
ggplot(data = Exposure_Chart,aes(x = factor(Sampling_Plan, levels =levels_2 ) ,y = MeanComparison, group = Baseline_Scenario,color = Baseline_Scenario))+
  theme_bw()+
  geom_point(size = 2.5)+
...
ggplot(data = Exposure_Chart,aes(x = factor(Sampling_Plan, levels =levels_2 ) ,y =Prevalence_Comparison, group = Baseline_Scenario,color = Baseline_Scenario))+
  theme_bw()+
  geom_point(size = 2.5)+
  #geom_col(data = Exposure_Chart, aes(x =factor(Sampling_Plan, levels =levels_2), y = log10cont))+
  facet_wrap(~Cont_Spread)+
